...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(API_CONFIG.getUrl("/api/auth/login"), {
        correo,
        contraseña,
      });
      if (response.data && response.data.token) {
        ...
        if (response.data.usuario) {
          ...
          // Si no viene el usuario en la respuesta, intentar obtenerlo del token
          try {
            ...
            const userData = {
              ...
              correo: tokenData.correo || tokenData.email,
            };
